...
- Either `seed_hsd_id` **or** a `signature` blob (error string, stack, or log excerpt).
- Optional filters: `tool` (`conformal`|`formality`), `tech`, `component`.

## Procedure

1. **Build signal set.**
   - If `seed_hsd_id` given: run `hsd-fetch-summarize` and copy its `Extracted FEV signals` block.
...
2. **Query HSDES MCP** with progressively broader queries:
   - Pass 1: exact unique substring(s) AND tool AND tech.
   - Pass 2: drop tech.
   - Pass 3: drop tool, keep core signature only.
   Stop early if Pass 1 already returns 5+ relevant hits.
3. **De-dupe** by ticket ID and by near-identical titles.
4. **Score** each candidate: +1 per matching signal (tool, sub_flow, tech, signature substring,
   component). Show top 5 by score.
5. For each survivor, extract the **proposed fix** from Resolution / RC / last meaningful comment.
   Quote, do not paraphrase.
6. **Aggregate**: list distinct fix strategies across the candidates, with counts.
